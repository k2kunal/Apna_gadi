...
const SignIn = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:5000/signin', formData); // API call to backend
      const { id, name } = res.data; // Assuming the response contains `id` and `name`

      // On successful login
      setMessage('Login successful!');
      localStorage.setItem('userData', JSON.stringify({ id, name })); // Store user data in localStorage
